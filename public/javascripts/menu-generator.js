...
(function($){
	$.fn.jsonMenuMaker = function (usrOption) {
		...
		var menuMaker = function (node) {
			...
			for (var i=0; i<node.length ; i++) {
				if (node[i].sub && !!opt.nested) {
					...
					output += '<li><a href="'+ node[i].href +'"><i class="fa fa-dashboard fa-fw"></i>'+ node[i].name +'<span class="fa arrow"></span></a><ul class="nav nav-second-level">'+subhtml+'</ul></li>\n';
				} else {
		...
		$.getJSON(opt.json, function(data) {
			...
			_this.html('<ul class="nav" id="side-menu">' + line + '</ul>');
		}).error(function(){
